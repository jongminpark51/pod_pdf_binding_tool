...
def get_watermark_font_name() -> str:
    for font_path in FONT_CANDIDATES:
        ...
        try:
            # TTFont??Í∏∞Î≥∏?ÅÏúºÎ°??ÑÎ≤†?©ÏùÑ ÏßÄ?êÌï©?àÎã§. (embed ?§Ïõå???Ä??Í∏∞Î≥∏ ?ôÏûë ?¨Ïö©)
            # ?¥Î†áÍ≤??¥Ïïº ??Î∏åÎùº?∞Ï? Î∑∞Ïñ¥?êÏÑú??Í∏Ä?êÍ? Íπ®Ï?ÏßÄ ?äÍ≥† Î≥¥ÏûÖ?àÎã§.
...
def read_pdf(uploaded_pdf: UploadedPdf) -> PdfReader:
    try:
        ...
        raise PdfBindingError(f"{uploaded_pdf.name}: PDF ?åÏùº???ΩÏùÑ ???ÜÏäµ?àÎã§.") from exc

    if reader.is_encrypted:
        raise PdfBindingError(
            f"{uploaded_pdf.name}: ?îÌò∏?îÎêú PDF??Ï≤òÎ¶¨?????ÜÏäµ?àÎã§."
        )
...
def build_binding_sample(
    ...
    if not pdfs:
        raise PdfBindingError("PDF ?åÏùº??1Í∞??¥ÏÉÅ ?†ÌÉù??Ï£ºÏÑ∏??")
...
def render_file_order(pdfs: list[UploadedPdf]) -> None:
    st.subheader("Î≥ëÌï© ?úÏÑú")
    sync_position_inputs(pdfs)

    with st.form("manual_order_form"):
        header_cols = st.columns([1, 5.4, 1.3])
        header_cols[0].caption("?úÏÑú")
        header_cols[1].caption("?åÏùºÎ™?)
        header_cols[2].caption("?¨Í∏∞")
        ...
        if st.form_submit_button("?´Ïûê ?úÏÑú ?ÅÏö©"):
...
def render_app() -> None:
    st.set_page_config(page_title=APP_NAME, layout="wide", page_icon="?êß")
    ...
    if ordered:
        ...
        if st.button("PDF ?ùÏÑ±", type="primary"):
            clear_generated_pdf()
            try:
                with st.spinner("PDF ?ùÏÑ± Ï§?):
                ...
                st.error(f"PDF ?ùÏÑ± Ï§??§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§: {exc}")
